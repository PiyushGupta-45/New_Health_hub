...
class _CommunityPageState extends State<CommunityPage> {
  ...
  Future<void> _loadMyCommunities() async {
    ...
    if (result['success'] == true && mounted) {
      setState(() {
        _myCommunities = List<Map<String, dynamic>>.from(result['data'] ?? []);
        _isLoading = false;
      });
    } else {
  ...
  Future<void> _loadPublicCommunities() async {
    ...
    if (result['success'] == true && mounted) {
      setState(() {
        _publicCommunities = List<Map<String, dynamic>>.from(
          result['data'] ?? [],
        );
      });
    } else {
  ...
  Future<void> _loadMessages() async {
    if (_selectedCommunity == null || _isLoadingMessages) return;
  ...
  Future<void> _createCommunity() async {
            ...
            content: Column(
              ...
              children: [
                RadioListTile<bool>(
                  ...
                  onChanged: (value) {
                    setState(() => selectedIsPublic = value!);
                  },
                ),
                RadioListTile<bool>(
                  title: const Text('Private'),
                  subtitle: const Text('Only people with code can join'),
                  value: false,
                  groupValue: selectedIsPublic,
                  onChanged: (value) {
                    setState(() => selectedIsPublic = value!);
    ...
    if (isPublic != null) {
      await _doCreateCommunity(name, isPublic!);
    }
  }

  Future<void> _doCreateCommunity(String name, bool isPublic) async {
    setState(() => _isLoading = true);
    ...
    if (result['success'] == true && mounted) {
      ...
      if (!isPublic && newCommunity['joinCode'] != null) {
        _showJoinCodeDialog(newCommunity['joinCode']);
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result['error'] ?? 'Failed to create community'),
          ),
        );
      }
    }
  ...
  Future<void> _joinPublicCommunity(String communityId) async {
    setState(() => _isLoading = true);
    ...
    if (result['success'] == true && mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result['error'] ?? 'Failed to join community'),
          ),
        );
      }
    }
  ...
  Future<void> _joinWithCode() async {
    ...
    final result = await _communityService.joinWithCode(code);
    if (result['success'] == true && mounted) {
  ...
  void _selectCommunity(Map<String, dynamic> community) {
    setState(() {
  ...
  void _showJoinCodeDialog(String joinCode) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Private Community Join Code'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(onPressed: _joinWithCode, child: const Text('Join')),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedCommunity != null) {
      return Scaffold(
        backgroundColor: const Color(0xFFF4F6F9),
        appBar: AppBar(
          title: Text(_selectedCommunity!['name'] ?? 'Community'),
          backgroundColor: Colors.indigo.shade600,
          foregroundColor: Colors.white,
          leading: IconButton(
              ...
              child: _isLoadingMessages && _messages.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isMe = message['userId']?.toString() == _userId;
                        return Align(
                          alignment: isMe
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (_myCommunities.isNotEmpty) ...[
                      ...
                      ..._myCommunities.map(
                        (community) => Card(
                          ...
                          elevation: 2,
                          child: ListTile(
                            title: Text(community['name'] ?? 'Untitled'),
                            subtitle: Text(
                    ...
                    if (_publicCommunities.isEmpty)
                      ...
                      ..._publicCommunities.map((community) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            trailing: isMember
                                ...
                                : ElevatedButton(
                                    onPressed: () =>
                                        _joinPublicCommunity(community['_id']),
